...
1. Symlinks this repo to `~/.cursor/plugins/local/clockify-dev` (not `clockify-agent-plugin`, so the published plugin id does not collide)
...
Verify: Plugins → `clockify-dev` (skills via `/`); MCP → `clockify-dev` green. This is not a consumer-install proof. Pre-allow tools as `clockify-dev:*` (not `clockify-agent-plugin:*`) — [use.md](./use.md#pre-enable-clockify-tools).
...
1. Removes `clockify-dev` / legacy `clockify` local plugin symlinks (old published id)
...
─────────────────────────      ─────────────────────────
plugin id: clockify-agent-plugin   plugin id: clockify-dev
