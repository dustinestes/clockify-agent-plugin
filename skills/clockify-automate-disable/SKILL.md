...
description: >-
  ...
  forge, on_start, triggers, runaway prefs, and modes. Distinct from
  clockify-unautomate (full rollback). Use when the user wants a pause they can
  resume with clockify-automate-enable.
disable-model-invocation: true
---

# Clockify automate disable

**Pause**, not a rollback. Confirm, then inert Cursor glue and flip live-automation flags off while **retaining** forge / `on_start` / triggers / runaway prefs / `platforms.cursor.modes`.
...
2. **Never automated** (init / after unautomate): `entry.automated.forge` is `none` (and typically empty triggers / empty modes). Chat that automate was never turned on (or was fully rolled back); suggest `/clockify-automate` if they want automation. Do not invent settings.
3. **Already paused** (`enabled: false` and `forge` is a real forge): still confirm and re-run glue removals (idempotent). Chat that settings are already paused; glue cleanup still runs if leftovers exist.
4. **Active** (`enabled: true` and real forge): normal disable path below.
...
2. Chat list: rule + Clockify-owned hooks (including `.cursor/hooks/clockify-runaway.sh` when present), and note that `entry.automated.enabled` and `platforms.cursor.enabled` will become `false` while **forge / on_start / triggers / runaway / modes stay**. Note empty dirs that will be removed if they become empty after cleanup. The managed `.clockify/` gitignore line stays. `plugin` / `scope` / `entry.timer` / `entry.manual` are not touched.
...
Patch `.clockify/config.yml` — do **not** wipe forge, `on_start`, `triggers`, `runaway`, or `modes`:
...
- Do **not** change `runaway.enabled` or `stop_after_minutes` (enable uses these to decide whether to reinstall hooks)
- Do **not** clear `triggers`, `forge`, `on_start`, or `modes`
...
- Clear forge / `on_start` / triggers / modes “for cleanliness”
- Flip `runaway.enabled` off (keep the user’s preference)
...
Automation is paused. Timer / stop / enter-time still work. Resume with `/clockify-automate-enable` (or re-run `/clockify-automate`, which skips wizards when forge settings are already present). Full rollback: `/clockify-unautomate`.
